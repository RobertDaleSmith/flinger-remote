...
function connectionSuccess() {
    ...
    //document.getElementById('intro_tv_url').innerHTML = "https://flinger.motelabs.com/#" + BGP.docname;

    document.getElementById('invite_buttons_url').innerHTML  = '<a href="https://flinger.motelabs.com/#' + BGP.docname + '" target="_blank">flinger.motelabs.com/#' + BGP.docname + '</a>';
    document.getElementById('invite_buttons_url2').innerHTML = '<a href="https://remote.flinger.motelabs.com/#' + BGP.docname + '" target="_blank">remote.flinger.motelabs.com/#' + BGP.docname + '</a>';

    //document.getElementById('invite_buttons_url').href = "https://flinger.motelabs.com/#" + BGP.docname;
...
function stateUpdated() {
    ...
    if (currentUrl != url) {
        ...
        if (video_id != 0) {
            ...
            document.getElementById("adblock_iframe").src = "https://flinger.motelabs.com/ad/?fling_url=" + url.replace("http://","").replace("https://","");
...
var storeInfoTitle = function (info) {
    ...
    document.getElementById('now_playing_video_title').href = "http://www.youtube.com/watch?v=" + snippet.id;

    document.getElementById("now_playing_thumbnail").src = snippet.thumbnails.default.url;

    document.getElementById("now_playing_thumbnail").href = "http://www.youtube.com/watch?v=" + snippet.id;
    ...
    document.getElementById("now_playing_video_uploader").textContent = snippet.channelTitle;

    // var gdata2 = document.createElement("script");
    // gdata2.src = "https://www.googleapis.com/youtube/v3/channels?id=" + snippet.channelId + "&key=" + BGP.YT_API_KEY + "&part=snippet&callback=storeVidInfoChannelThumb";
...
function inviteFacebookButton() {
    ...
    open('http://www.facebook.com/share.php?u=https://flinger.motelabs.com/%23' + BGP.docname, 'facebook', 'toolbar=no,width=700,height=400');
    //window.open('http://www.facebook.com/share.php?u=https://flinger.motelabs.com/%23' + BGP.docname,'_blank');
}
function inviteTwitterButton() {

    open('http://twitter.com/share?text=Join%20Flinger%20Channel%20%23'+ BGP.docname +'%20to%20watch%20and%20Fling%20videos%20with%20me%20%40flingerco&url=https://flinger.motelabs.com/%23' + BGP.docname, 'twitter', 'toolbar=no,width=700,height=400');
    //window.open('http://twitter.com/share?text=Join%20Flinger%20Channel%20%23'+ BGP.docname +'%20to%20watch%20and%20Fling%20videos%20with%20me%20%40flingerco&url=https://flinger.motelabs.com/%23' + BGP.docname,'_blank');
}
function inviteGooPlusButton() {
    
    open('https://plus.google.com/share?url=https://flinger.motelabs.com/%23' + BGP.docname, 'g-plus', 'toolbar=no,width=700,height=400');
    //window.open('https://plus.google.com/share?url=https://flinger.motelabs.com/%23' + BGP.docname,'_blank');
}
...
var connectAniLoop = function () {
    if (!connectAniActive) {
        ...
        $('#boom_1').animate({ rotate: '+=1080deg', scale: '.52', top: "+=950px", left: "+=950px" }, 2400);
        $('#boom_2').animate({ rotate: '+=1080deg', scale: '.52', top: "+=0px", left: "-=1050px" }, 2400);
        $('#boom_3').animate({ rotate: '+=1080deg', scale: '.52', top: "-=950px", left: "+=950px" }, 2400, function () { connectAniActive = false; });
    }
}
...
var initializeUIElements = function () {
    ...
    $("#sync_adverts_button").click(function () { toggleConfigMenu(); window.open("https://flinger.motelabs.com/advertise-on-flinger", "_blank"); toggleMainMenu(); });
    ...
    document.getElementById("adblock_iframe").src = "https://flinger.motelabs.com/ad/";
...
var getSeconds = function (iso8601String) {
    var reptms = /^PT(?:(\d+)H)?(?:(\d+)M)?(?:(\d+)S)?$/;
    var hours = 0, minutes = 0, seconds = 0, totalseconds;

    if (reptms.test(iso8601String)) {
        var matches = reptms.exec(iso8601String);
...
var setQueItemDetails = function (url, queItemDiv) {
    ...
    if (youtube_id != 0) {
        ...
        $.getJSON("https://www.googleapis.com/youtube/v3/videos?id=" + youtube_id + "&key=" + BGP.YT_API_KEY + "&part=contentDetails,snippet,statistics", function (info) {
            if (!(info && info.items && info.items.length)) return;
            ...
            var durationTime = secondsToTime(getSeconds(contentDetails.duration));
